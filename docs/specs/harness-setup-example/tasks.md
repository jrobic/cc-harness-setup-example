...
- [x] **T-c1** Write `plugins/.../hooks/hooks.json`: SessionStart nudge invoking
      ...
      _Depends:_ T-b8
- [x] **T-c2** Write `scripts/build-hardened.ts`: `bun build --compile --outfile
  dist/harness-setup` (document `--target bun-<os>-<arch>` for cross builds);
      wire `build:hardened` script. (ADR-0003, R10.2) _Depends:_ T-b8
- [x] **T-c3 (opt)** Write `scripts/set-mode.ts`: flip hook/command invocation
      strings between soft and hardened. (ADR-0003) _Depends:_ T-c1, T-c2
- [x] **T-c4** Verify hardened path: build the binary, run `check`/`apply`
...
- [x] **T-e1** Write `plugins/.../commands/harness-setup.md` (EN): locate engine
...
- [x] **T-i2** Do NOT ship `BRIEF.md`; run a de-internalisation grep gate over the
...
- [x] **T-j1 (opt)** `examples/clone-and-go/.claude/settings.json`: minimal repo
      skeleton showing `extraKnownMarketplaces` onboarding. _Depends:_ T-a6, T-i1
- [x] **T-j2 (opt)** `PRESENTATION.md`: talk track — three layers, deny ≠ context,
